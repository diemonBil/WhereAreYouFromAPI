...
from name_origin.models import Country, Name, NameCountryStat, CountryBorder
...
class NameStatsViewTest(APITestCase):
    def setUp(self):
        ...
        self.country = Country.objects.create(code="US", name="United States", alpha3="USA")

        # Pre-create a name with a recent timestamp to test cache
        self.name = Name.objects.create(value="John", count_of_requests=2)
        NameCountryStat.objects.create(name=self.name, country=self.country, probability=0.75)
...
class PopularNamesViewTest(APITestCase):
    def setUp(self):
        ...
        NameCountryStat.objects.create(name=name1, country=self.country, probability=0.85)
        NameCountryStat.objects.create(name=name2, country=self.country, probability=0.55)

    def test_popular_names_success(self):
        # Should return top 5 names sorted by probability
        response = self.client.get('/api/v1/popular-names/?country=US', **self.auth_headers)
    ...
    def test_nonexistent_country(self):
        ...
        response = self.client.get('/api/v1/popular-names/?country=ZZ', **self.auth_headers)
