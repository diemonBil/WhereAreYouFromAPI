...
class Name(models.Model):
    ...
    last_accessed = models.DateTimeField(auto_now=True)  # Automatically updated each time the record is saved
...
class Country(models.Model):
    ...
    capital_lat = models.FloatField(blank=True, null=True)  # Latitude of the capital city
    capital_lng = models.FloatField(blank=True, null=True)  # Longitude of the capital city
...
class CountryBorder(models.Model):
    from_country = models.ForeignKey(
        ...
        related_name='borders_from'
    )
    to_country = models.ForeignKey(
        'Country',
        on_delete=models.CASCADE,
        related_name='borders_to'
    )
    ...
    def save(self, *args, **kwargs):
        # Normalize the pair direction to avoid duplicates like (UA→PL) and (PL→UA)
...
class NameCountryStat(models.Model):
    name = models.ForeignKey(Name, on_delete=models.CASCADE, related_name='country_stats')
    country = models.ForeignKey(Country, on_delete=models.CASCADE, related_name='name_stats')
    probability = models.FloatField(
        validators=[MinValueValidator(0.0), MaxValueValidator(1.0)]  # Restrict values to range 0.0–1.0
    )
